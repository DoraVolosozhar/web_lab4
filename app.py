...
def post():
    ...
    try:
        if request.method == 'GET':
            ...
            path = "static/"+name_date[0]+"/"
            if os.path.exists(path) and os.path.isdir(path):
                shutil.rmtree(path)
            os.system(f"Instaloader  --post-filter=\"date_utc <= datetime({f_data}) and date_utc >= datetime({s_data})\" --dirname-pattern {path} --no-metadata-json --no-videos {name_date[0]}")
